...
namespace SharpAdbClient
{
    ...
    public interface IAdbClient
    {
		...
		/// <summary>
		/// Asks the ADB server to reverse remote connections from <paramref name="local"/>
		/// to the <paramref name="remote"/> host address on the <paramref name="device"/>.
		/// </summary>
		/// <param name="device">
		/// The device to which to forward the connections.
		/// </param>
		/// <param name="local">
		/// <para>
		/// The local address to forward on the device. This value can be in one of:
		/// </para>
		/// <list type="ordered">
		///   <item>
		///     <c>tcp:&lt;port&gt;</c>: TCP connection on localhost:&lt;port&gt;
		...
		/// <param name="remote">
		/// <para>
		/// The remote address to forward to on the host. This value can be in one of:
		/// </para>
		/// <list type="ordered">
		///   <item>
		///     <c>tcp:&lt;port&gt;</c>: TCP connection on localhost:&lt;port&gt; on device
		...
		/// which has been opened. In all other cases, <c>0</c>.
		/// </returns>
		bool CreateReverse(DeviceData device, string local, string remote, bool allowRebind);
		...
		/// which has been opened. In all other cases, <c>0</c>.
		/// </returns>
		bool CreateReverse(DeviceData device, ForwardSpec local, ForwardSpec remote, bool allowRebind);

		/// <include file='IAdbClient.xml' path='/IAdbClient/RemoveForward/*'/>
		bool RemoveReverse(DeviceData device, int localPort);

		/// <include file='IAdbClient.xml' path='/IAdbClient/RemoveAllForwards/*'/>
		bool RemoveAllReverse(DeviceData device);

		/// <include file='IAdbClient.xml' path='/IAdbClient/ListForward/*'/>
		IEnumerable<ForwardData> ListReverse(DeviceData device);
